...
// init
const duplicateClasses: Map<SVGClasses, string[]> = new Map([
    [SVGClasses.Arrow, []],
    [SVGClasses.StateBox, []],
]);
...
function getUniqueClass(className: string, elementType: SVGClasses): string {
...
async function generateTypeEnums(outputSVGTypesFile: string): Promise<void> {
    const arrowClasses = duplicateClasses.get(SVGClasses.Arrow) ?? [];
    const stateBoxClasses = duplicateClasses.get(SVGClasses.StateBox) ?? [];
